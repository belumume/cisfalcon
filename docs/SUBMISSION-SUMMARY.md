...
**In one line:** rank your AI-designed enhancers safest-first and cut specificity failures by about 41% within one target cell and one generator, which is the deployment case (about 70% pooled across a mixed batch, but a sequence-free stratum-prior rule beats that pooled figure at about 90%, so 41% is the honest one),
...
- The operating point that matters is triage, not a hard gate. As a uniform abort-gate it
  ...
  batch and 49% of those truly fail, a 7.74x enrichment over the 6.29% base rate POOLED. Conditioned within (cell x generator), which is how a lab deploys it, that is 2.59x; a sequence-free stratum-prior rule beats the pooled safest-half headline (about 90% vs 70%), so the conditioned figures are the honest ones. On a
