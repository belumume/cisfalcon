...
def fit_eval(fit, ev):
    """Isotonic fit on `fit`, binned reliability + ECE evaluated on `ev`."""
    iso = IsotonicRegression(increasing=False, out_of_bounds="clip").fit(
        fit["pred_gap"].values, fit["fail"].values
    )
    p = iso.predict(ev["pred_gap"].values)
    y = ev["fail"].values.astype(float)
    ...
    return np.array(conf), np.array(obs), np.array(cnt), ece


# The prose everywhere else quotes 0.0031, which is the MEAN of the two split directions.
# This panel can only plot one of them, so compute both and title with the mean plus the
# panel's own value. Titling with the panel value alone (0.0029) sent a reader who opened
# the cited figure to check 0.0031 to a different, more favourable number.
conf, obs, cnt, ece = fit_eval(A, B)
_, _, _, ece_rev = fit_eval(B, A)
ece_mean = (ece + ece_rev) / 2
...
ax.set_title(
    ...
    f"held-out ECE = {ece_mean:.4f}  (mean of both split directions)\n"
    f"this panel: fit-A/eval-B, ECE {ece:.4f}, n = {len(B):,} cross-lab designs",
    fontsize=9,
)
