...
function renderBatchResult(rows, R) {
  ...
  </div><div class="dim" style="font-size:11px;margin-top:6px">${flaggedInRiskiestHalf} of ${nfail} predicted failures sort into the riskiest half here, which is by construction rather than a measured result: the ranking and the pass/fail call both come from the same predicted gap, and your library carries no wet-lab labels to score against. The <i>measured</i> safest-half reduction, on the 93,435-design cross-lab benchmark, is <b>41%</b> conditioned within (cell &times; generator) and 70% pooled, where a sequence-free stratum-prior rule reaches about 90%.</div><table><thead><tr><th>rank</th><th>design</th><th>gap</th><th>most-active</th><th>call</th></tr></thead><tbody>`;
